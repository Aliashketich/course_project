...
public class ShowOrderWaiter implements ICommand {
    private static final Logger LOGGER = Logger.getLogger(ShowOrderWaiter.class);
    private ServiceFactory serviceFactory = ServiceFactory.getInstance();
    private PageName pageName = PageName.ORDERS;

    @Override
    public String execute(HttpServletRequest request, HttpServletResponse response) {
        LOGGER.log(Level.INFO, "Command: ShowOrderWaiter started.");
        try {
            int idWaiter = ((Waiter) request.getSession().getAttribute("waiter")).getIdUser();
            List<Order> orders = serviceFactory.getOrderService().findAllOrdersByWaiterId(idWaiter);
            List<OrderProduct> orderProducts = serviceFactory.getOrderProductService().findOrderProductsByWaiterId(idWaiter);
        ...
        LOGGER.log(Level.INFO, "Command: ShowOrderWaiter finished.");
